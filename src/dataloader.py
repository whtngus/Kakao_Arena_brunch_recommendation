...
class DataLoader:
    ...
    def kakao_data_loader(self,target_path):
        try:
            with open(target_path, "r") as target_file:
                self.kakao_data = target_file.readline().split(" ")
                target_file.close()
        except FileNotFoundError as e:
            print("해당 파일이 존재하지 않습니다.")
            print(e)
    ...
    def ngram_data_loader(self, days):
        try:
            with open(self.data_path["factory_path"] + "/" + str(days) + "result_unigram.json", "rb") as unigram_file:
                # self.unigram = ijson.parse(unigram_file.read())
                self.unigram = pickle.load(unigram_file)
                unigram_file.close()

            with open(self.data_path["factory_path"] + "/" + str(days) + "result_bigram.json", "rb") as bigram_file:
                # self. bigram = ijson.parse(bigram_file.read())
                self.bigram = pickle.load(bigram_file)
                bigram_file.close()

            with open(self.data_path["factory_path"] + "/" + str(days) + "result_trigram.json", "rb") as trigram_file:
                # self.trigram = ijson.parse(trigram_file.read())
                self.trigram = pickle.load(trigram_file)
                trigram_file.close()
